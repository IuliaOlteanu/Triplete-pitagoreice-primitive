...
;generator 
(define (make-triple Q)
  (let*(
       (elemente_1 (multiply T1 (stream-first Q)))
       (elemente_2 (multiply T2 (stream-first Q)))
       (elemente_3 (multiply T3 (stream-first Q)))
       (lista (list elemente_1 elemente_2 elemente_3))
       (my_Q (stream-append Q lista))
       )
    (stream-cons (stream-first Q) (make-triple (stream-rest my_Q)))))
...
(define (pairs G H)
 (let pereche((i 0) (j 0) (my_G G) (my_H H))
    (cond
      ((< i j) (stream-cons (creare my_G my_H)
                            (pereche (add1 i) j (stream-rest my_G)  my_H)))
      ((= i j) (stream-cons (creare my_G my_H)
                            (pereche 0 (add1 j) G (stream-rest my_H))))
      )))
...
(define (make-odds k)
  (stream-cons k (make-odds (+ k 2))))

;G = 1 3 5 7 ...
(define G (make-odds 1))

;H = 3 5 7 9 ...
(define H (make-odds 3))


(define gh-pairs-stream
  (let(
      (pereche (pairs G H)))
   (stream-filter (λ(pair)
                     (= (gcd(car pair) (cdr pair)) 1)) pereche)))
...
(define ppt-stream-in-pair-order
  (stream-map(lambda(x)
                (let(
                     (g (car x))
                     (h (cdr x)))
                  (list (my_a g h) (my_b g h) (my_c g h))))
              gh-pairs-stream ))
