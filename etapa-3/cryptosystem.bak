...
(define (key n)
  (let*(
       ...
       (rez (apply list (list (a1 e f) (b1 e f) (c1 e f) (a2 e f) (b2 e f) (c2 e f)
          (a3 e f) (b3 e f) (c3 e f))))
       )
    (map (lambda(element)
           (if(and (<= element 26) (>= element 0))
              element
              (modulo element 27))) rez)))
...
(define (message->codes message)
  (let(
        (lista (string->list message))
        )
    (map (lambda(element)
            (modulo(char->integer element) 32)) lista)))
...
(define (codes->message codes)
 (list->string(map (lambda(element)
      (if(zero? element)
          (integer->char 32)
          (integer->char(+ 96 element))))codes)
       ))
...
(define (extend-key key size)
  (let(
    ...
    (if(<= dim_key size)
       (append rez (extend-key key (- size dim_key)))
       (take rez size))
  ))
...
;functie : + / -
(define (general-funct-codes functie)
    (lambda(L1 L2)
        (let(
             (dim_1 (length L1))
             (rez null))
          (map(lambda(elem1 elem2)
                (append rez(modulo(functie elem1 elem2) 27)))L1 (extend-key L2 dim_1)))))
...
;(message->codes message)intoarce o lista de coduri
;(codes->message codes) intoarce un mesaj
(define (general-funct-message functie)
    (lambda(mesaj L2)
        (let*(
              (lista_coduri (message->codes mesaj))
              (cheie_extinsa (extend-key L2 (length lista_coduri))))
    
          (codes->message(functie lista_coduri cheie_extinsa))
        )))
       
(define encrypt-message 
  (general-funct-message encrypt-codes))
 
(define decrypt-message 
  (general-funct-message decrypt-codes))
