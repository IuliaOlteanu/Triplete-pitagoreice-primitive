...
(define (inmultire M V rez)
  (if (null? M)
      rez
      (inmultire(cdr M) V (append rez (list (dot-product (car M) V))))))
...
(define (fct Ts ppt rez)
  (cond
    ((null? Ts) rez)
    ((= (car Ts) 1) (fct(cdr Ts) (multiply T1 ppt)  (multiply T1 ppt)))
    ((= (car Ts) 2) (fct(cdr Ts) (multiply T2 ppt) (multiply T2 ppt)))
    (else (fct(cdr Ts) (multiply T3 ppt) (multiply T3 ppt)))))
